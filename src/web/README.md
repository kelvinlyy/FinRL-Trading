...
| Element                                                          | Data Source                                                       | Status                                   |
| ---------------------------------------------------------------- | ----------------------------------------------------------------- | ---------------------------------------- |
| Adaptive Rotation runner                                         | `deploy.sh` / `run_adaptive_rotation_strategy.py` with Yahoo CSVs | Real                                     |
| Enhanced chart display                                           | `enhanced_backtest_*.png` generated by the visualizer             | Real                                     |
| Trade log/results browser                                        | `trade_log_*.csv`, `backtest_*.csv` output files                  | Real                                     |
| Legacy strategy selector (equal_weight, market_cap_weight, etc.) | `BacktestEngine` with **synthetic random price data**             | Mock / TBD                               |
| Legacy results: key metrics, equity chart, detailed table        | Computed from legacy mock backtest result                         | Mock / TBD                               |


**Key limitation:** The Adaptive Rotation workflow is the real strategy-backed flow. The legacy generic backtest tab is retained as a demo and marked **Mock / TBD**.
...
| Adaptive Rotation backtest | —              | Yahoo CSVs + strategy engine + visualizer     |
| Legacy generic backtest   | Random walk    | —                                             |
...
| **Data**         | Mixed: Adaptive Rotation real, other sections mock | Real backtest results                         |
...
1. **Mock / TBD sections remain:** Overview, Portfolio Analysis, Data Quality, and the legacy generic backtest demo still use demo/random/static values and are labeled in the UI.
2. **Strategy Execution tab:** References `EqualWeightStrategy` which is not defined in `base_strategy.py`; this tab is labeled Mock / TBD.
3. **Settings persistence:** None of the settings changes are saved to disk or `.env`.
4. **Adaptive Rotation runtime:** The real backtest workflow shells out to `deploy.sh`, so long date ranges can take several minutes and depend on local Yahoo CSV availability unless download is enabled.
